...
class FatHandler:
    ...
    def get_cluster_entry_in_fat(self, cluster_number: int, table_number: int) -> int:
        ...
        if self.fat_type == FatType.fat16:
            fat_offset = cluster_number * FatHandler.LENGTH_CLUSTER_FAT16
        else:
            fat_offset = cluster_number * FatHandler.LENGTH_CLUSTER_FAT32
    ...
    def get_cluster_entry_in_data(self, cluster_number: int) -> int:
        first_sector = (self.fat_entity.first_data_sector +
                        (cluster_number - 2) * self.fat_entity.BPB_SecPerClus)
    ...
    def get_cluster_value_in_main_fat(self, cluster_number: int) -> int:
        return self.get_cluster_value_in_fat(cluster_number, 0)

    def is_final_cluster(self, fat_cluster_value: int) -> bool:
    ...
    def write_cluster_value_in_all_tables(self, value: int, cluster_number: int) -> None:
        for i in range(self.fat_entity.BPB_NumFATs):
            self.write_cluster_value_in_fat(value, cluster_number, i)

    def write_cluster_value_in_fat(self, value: int, cluster_number: int, fat_number: int) -> None:
        entry = self.get_cluster_entry_in_fat(cluster_number, fat_number)
    ...
    def get_cluster_value_in_data(self, cluster_number: int) -> bytes:
        entry = self.get_cluster_entry_in_data(cluster_number)
