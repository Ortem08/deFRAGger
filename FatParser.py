...
def parse_fat(file_handler: FileHandler):
    ...
    error_detector = ErrorHandler(fat_handler)
    directory_parser = DirectoryParser(fat_handler)

    if error_detector.check_differences_in_fats():
        return FileSystem(far_entity, fat_handler, {}, error_detector)

    fat_index = FatIndexer(directory_parser)
    full_indexed_fat_table = fat_index.get_full_indexed_table()

    if error_detector.is_looped_or_intersecting_files(full_indexed_fat_table):
        return FileSystem(far_entity, fat_handler, full_indexed_fat_table, error_detector)

    correct_indexed_fat_table = fat_index.get_correct_indexed_table()

    file_system = FileSystem(far_entity, fat_handler, correct_indexed_fat_table, error_detector)
