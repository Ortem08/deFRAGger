...
class FatEntity:
    def __init__(self, file_handler: FileHandler):
        ...
        self.RootDirSectorsCnt = (((self.BPB_RootEntCnt * 32) +
                                  (self.BPB_BytsPerSec - 1)) //
                                  self.BPB_BytsPerSec)

        # Начало дата сектора, первый сектор второго кластера
        self.first_data_sector = (self.BPB_ResvdSecCnt + self.RootDirSectorsCnt
                                  + (self.BPB_NumFATs * FATSz))
    ...
    def get_fat_type(self) -> FatType:
        if self.clusters_count <= 0:
            raise ValueError(f"Некорректное число кластеров: "
                             f"{self.clusters_count}")
